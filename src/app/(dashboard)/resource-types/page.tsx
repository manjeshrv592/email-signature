...
export default async function ResourceTypesPage({
  searchParams,
}: ResourceTypesPageProps) {
  const { page } = await searchParams;
  const currentPage = Math.max(1, parseInt(page || "1", 10));
  const offset = (currentPage - 1) * PAGE_SIZE;

  // Get total count
  const [{ total }] = await db.select({ total: count() }).from(resourceTypes);
      ...
      <div className="flex items-start justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-white flex items-center gap-3">
            <div className="inline-flex items-center justify-center w-10 h-10 rounded-xl bg-linear-to-br from-violet-500 to-purple-500 shadow-lg">
      ...
      {total === 0 ? (
        <div className="bg-[#12121a] border border-[#1e1e2e] rounded-2xl p-12 text-center">
          ...
          <p className="text-[#555] text-sm max-w-md mx-auto mb-6">
            Resource types define the structure for your signature components —
            like certifications, banners, or legal text. Each type has custom
            fields you define.
          </p>
          <Link
            href="/resource-types/new"
